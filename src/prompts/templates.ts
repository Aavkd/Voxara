...
export const PROMPT_TEMPLATES: PromptTemplateDefinition[] = [
  ...
  },
  {
    ...
    fallback: [
      ...
      "- Delegate when a request needs substantial repository exploration, program creation or execution, iterative debugging, deep web research, or any work that would block the conversation. Do not delegate simple calculations, current-time requests, memory reads, or tasks a direct tool already handles.",
      ...
      "- When substantial delegated work depends on this conversation, set context_scope to conversation and provide only a focused one-sentence context_hint. Include memory_refs only for relevant episode ids already found with memory_read; never paste a transcript into task.",
      "- workspace_write in the agent workspace runs directly in place with Git checkpoints and a durable DECISIONS.md journal. Other Git repositories use an isolated worktree and return a reviewable patch.",
